...
class PluginsLoader {
    ...
    for (const i of priority) {
      if (i.plugin.accept) {
        try {
          ...
          Bot.makeLog("error", [`${logger.red(i.name)}`, err], 'Accept')
          break
        }
      }
    }
  ...
  dealEvent(e, groupCfg) {
    ...
    Object.defineProperty(e, "isMaster", {
      get() {
        return (e.user_id && cfg.master[e.self_id]?.includes(String(e.user_id)))
      },
      set(v) {
        logger.warn(`⚠️ 已拦截 e.isMaster 的非法修改! user_id: ${e.user_id} isMaster = ${v}`)
        logger.warn("修改 e.isMaster 的调用栈：\n", new Error().stack.split('\n').slice(1).join('\n'))
      },
      configurable: false,
      enumerable: true
    })
